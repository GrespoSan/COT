...
# =============================================================================
st.set_page_config(
    page_title="COT Smart Money V6.9 — Python",
    page_icon="🛡️",
    layout="wide",
)
...
st.title("🛡️ COT Smart Money — Python V6.9")
st.caption(
...
def build_history_df(
    ...
    # Due orizzonti informativi fissi, identici a TradingView V1.5.22.
...
def cot_zone(index_value: float) -> str:
    """Classificazione identica a TradingView V1.5.22."""
    if pd.isna(index_value):
        return "NON DISPONIBILE"
    if index_value >= 80:
        return "POSIZIONAMENTO LONG ESTREMO"
    if index_value >= 60:
        return "POSIZIONAMENTO LONG ELEVATO"
    if index_value > 40:
        return "POSIZIONAMENTO NELLA MEDIA"
    if index_value > 20:
        return "POSIZIONAMENTO SHORT ELEVATO"
    return "POSIZIONAMENTO SHORT ESTREMO"
...
def analyze_smart_money(
    ...
    # TESTO SEMPLICE ULTIMO REPORT — identico a TradingView V1.5.22
    if new_long:
    ...
    # =========================================================================
    # LETTURA SEMPLICE — FRASI IDENTICHE A TRADINGVIEW V1.5.22
    # =========================================================================
    ...
    # =========================================================================
    # SINTESI CHIARA DEL QUADRO — identica alla V1.5.22
    # =========================================================================
    exposure_long_side = extreme_long or (not pd.isna(cot_index) and 60 <= cot_index < 80)
    exposure_short_side = extreme_short or (not pd.isna(cot_index) and 20 < cot_index <= 40)
    position_neutral = not pd.isna(cot_index) and 40 < cot_index < 60

    if exposure_long_side:
        ...
        exposure_summary = f"I {spec.trend_label} mantengono un'esposizione neutra"
    ...
    conjunction = "; nell'ultimo report " if position_neutral else ", ma nell'ultimo report " if last_change_opposes_exposure else " e nell'ultimo report "
    ...
    clear_summary = f"{funds_summary} {counter_summary} {price_summary}"
...
def build_screener_excel(results: pd.DataFrame, errors: pd.DataFrame) -> bytes:
    ...
    display_columns = [
        ...
        "COT Index", "COT Index 26W", "COT Index 156W", "Esposizione Long %", "Esposizione Short %",
        "Alignment rialzista", "Alignment ribassista", "Rapid Shift 6W",
        "Variazione OI %", "OI Index 52W", "Prezzo Weekly", "Concentrazione Top 8", "Data COT",
    ]
...
def build_screener_ai_prompt(top_rows: pd.DataFrame, top_n: int) -> str:
    ...
    compact_columns = [
        ...
        "COT Index", "COT Index 26W", "COT Index 156W", "Posizione attuale", "Esposizione Long %", "Esposizione Short %",
        "Flow 1W", "Flow 3W", "Flow 6W", "Rapid Shift 6W", "Stato Rapid Shift",
        "Variazione OI %", "OI Index 52W", "Stato OI 52W",
